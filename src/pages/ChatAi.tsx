...
const ChatAi: React.FC = () => {
  ...
  const currentConversation = filteredConversations.find((conv) => conv.id === currentConversationId);
  ...
  const handleSendMessage = (text: string, conversationId: string) => {
    ...
    setTimeout(() => {
      ...
      const typingDuration = aiResponseText.length * 50 + 500; // 50ms per char + 500ms base
      ...
      setTimeout(() => {
        ...
        setAllConversations((prev) =>
          prev
            .map((conv) =>
              conv.id === conversationId
                ? { ...conv, messages: [...conv.messages, aiFinalMessage], lastActivity: new Date() }
                : conv,
            )
        ...
        setFilteredConversations(
          allConversations.filter(
            (conv) =>
              ...
              conv.messages.some((msg) => msg.text.toLowerCase().includes(lowerCaseTerm)),
          ),
        );
      }
    },
  ...
  const handleDeleteConversation = (idToDelete: string) => {
    setAllConversations((prev) => prev.filter((conv) => conv.id !== idToDelete));
    ...
    if (currentConversationId === idToDelete) {
      ...
      const remainingConversations = allConversations.filter((conv) => conv.id !== idToDelete);
  ...
  useEffect(() => {
    const currentSearchTerm =
      (document.querySelector('input[placeholder="Tìm kiếm đoạn chat..."]') as HTMLInputElement)?.value || "";
    ...
    } else if (currentConversationId && !allConversations.find((c) => c.id === currentConversationId)) {
