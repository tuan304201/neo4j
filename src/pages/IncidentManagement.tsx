...
const IncidentManagement: React.FC = () => {
  ...
  const fetchAlerts = async () => {
    ...
    try {
      ...
      if (activeTab === "Active") {
        filteredAlerts = fetchedAlerts.filter((alert) => alert.status !== "Resolved" && alert.status !== "Suppressed");
    ...
    try {
      ...
      if (success) {
        if (activeTab === "Active") {
          ...
          setAlerts((prev) => prev.map((a) => (a.id === alert.id ? { ...a, status: "Suppressed" } : a)));
        }
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-display font-bold text-gray-900 dark:text-white">Incident Management</h1>
