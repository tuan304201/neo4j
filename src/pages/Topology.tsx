...
import { 
  GraphData, GraphFilterOptions, GraphLayoutOptions, Node, Edge 
} from '../types/topology.types';
import { 
  getTopologyGraph, getNodeById, getNodeConnections, getRelatedNodes, searchNodes
...
const Topology: React.FC = () => {
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      ...
      if (filterOptions.nodeTypes.length > 0 || filterOptions.status.length > 0) {
        const filteredMatchingNodes = matchingNodes.filter(node => {
          const matchesType = filterOptions.nodeTypes.length === 0 || 
        ...
        const relevantEdges = graphData.edges.filter(edge => 
          nodeIds.includes(edge.source) && nodeIds.includes(edge.target) &&
          (filterOptions.edgeTypes.length === 0 || filterOptions.edgeTypes.includes(edge.type))
        );
        ...
        <div className="mt-6">
          {loading ? (
            <div className="h-[600px] flex items-center justify-center">
              ...
              <p className="ml-2 text-gray-600 dark:text-gray-300">Loading topology...</p>
            </div>
          ) : (
            <TopologyGraph
              data={filteredData}
